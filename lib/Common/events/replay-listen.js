...
function estimateEventJsonBytes(ev) {
    return (0, wire_size_1.jsonUtf8ByteLength)(ev.event);
}
...
function decorateReplayListen(base, options, deliveryControl) {
    const { current: currentOpt, frame: condense, history = 0, keepMs, keepBytes, sizeOf, getSince, onJournal, onJournalBatch, now = Date.now, staleMs, onStale, firstSeq = 0, } = options;
    ...
    function measureEventBytes(ev) {
        let size;
        try {
            size = measure(ev);
        }
        catch {
            return UNMEASURED_EVENT_BYTES;
        }
...
function replayListen(options = {}) {
    const { current, frame, history, keepMs, keepBytes, sizeOf, getSince, onJournal, onJournalBatch, now, staleMs, onStale, firstSeq, ...listenOptions } = options;
    let t;
