...
function exposeStoreReplayBatchV7(replay, prepareRead) {
    ...
    function frame(seq, hint, snapshot) {
        prepareRead();
        const knowledge = codec.createRemoteKnowledge(snapshot);
...
function decodeStoreReplayRemote(remote) {
    ...
    function selectedRemote() {
        ...
        if (codec == 'v7') {
            ...
            activeRemote = decodeStoreReplayWireRemote(remote.v7, wire => msgpack.decode(wire), remote, msgpack.knowledge);
        }
        else if (codec == 'v6') {
