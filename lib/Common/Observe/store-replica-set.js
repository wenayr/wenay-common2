...
function createStoreReplicaSet(deps) {
    ...
    function pushedSeqAdvance(entry, pushed) {
        const current = entry.descriptor;
        if (!pushed || entry.state != 'open' || !current)
            return null;
        let value;
        try {
            value = validateDescriptor(pushed);
        }
        catch {
            return null;
        }
        if (value.headSeq < current.headSeq || value.authoritySeq < current.authoritySeq)
            return null;
        const seqFree = { headSeq: 0, authoritySeq: 0 };
        if (!(0, common_1.deepEqual)({ ...value, ...seqFree }, { ...current, ...seqFree }))
            return null;
        return { ...value, path: [...value.path] };
    }
    function refreshEntry(entry, pushed) {
        if (entry.refreshing)
    ...
    async function openEntry(entry) {
        ...
        try {
            ...
            if (closed || generation != entry.generation || entry.session != session)
                return;
            if (hasChanged) {
                entry.offChanged = session.remote.changed.on(function remoteDescriptorChanged(pushed) {
    ...
    function setOffers(next) {
        ...
        if (repriced)
            scheduleReconcile('offer repriced');
    }
