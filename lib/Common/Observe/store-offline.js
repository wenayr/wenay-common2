...
async function loadSnapshot(opts) {
    ...
    if (migrate)
        return { snapshot: await migrate(raw, 0, version), seq: -1, replayMode: 'legacy' };
    return { snapshot: initial, seq: -1, replayMode: 'legacy' };
}
async function createOfflineStore(opts) {
    ...
    async function runReconnect(generation, cancelled, nextRemote, nextSyncOpts) {
        ...
        try {
            const schemaReady = effectiveSyncOpts.batch ? (0, transport_lifecycle_1.getRpcSchemaReady)(nextRemote) : undefined;
            if (schemaReady) {
            ...
            if (!isCurrent())
                return;
            const mode = (0, store_replay_1.storeReplayMode)(nextRemote, effectiveSyncOpts.batch);
