...
function followReplicatedMap(remote, opts = {}) {
    const { initial, drain, onBatch, onStatus, delivery: requestedDelivery, batch = true, checkpoint, onSeq, onError, onLive, onStale, validateBatch, ...wireOpts } = opts;
    ...
    async function prepareReplicatedMapCatchUp(context) {
        ...
        const nextMode = (0, store_replay_1.storeReplayMode)(remote, batch);
        ...
        if (delivery == 'lossless') {
            throw new Error(`lossless replicated map replay line changed ${phase}: ${lineId} -> ${next?.lineId ?? 'legacy'}`);
        }
        lineId = next?.lineId ?? '';
        setStatus({ seq: -1 });
        return { reset: true };
    ...
    async function start() {
        ...
        const replayMode = (0, store_replay_1.storeReplayMode)(remote, batch);
        ...
        if (requestedCursor && requestedCursor.lineId != lineId && delivery == 'lossless') {
            reportSyncError(new Error(`lossless replicated map cannot resume another replay line: ${requestedCursor.lineId} -> ${lineId || 'legacy'}`));
            return;
        }
    ...
    function checkpointSnapshot() {
        if (!lineId)
            throw new Error('legacy replicated map remote has no safe checkpoint identity');
        return {
