...
exports.openDurableStore = openDurableStore;
...
function openDurableStore(deps) {
    ...
    function attach(line) {
        ...
        return {
            ...
            retry,
            flush() {
                (0, reactive_1.flushReactiveNow)(store.state);
                line.flushPending?.();
            },
            close() {
                (0, reactive_1.flushReactiveNow)(store.state);
...
function createDurableStoreReplay(deps) {
    ...
    const opened = openDurableStore({ ...storeDeps, ...(userOnJournal ? { onJournal: userOnJournal } : {}) });
    const exposed = (0, store_replay_1.exposeStoreReplay)(opened.store, { ...exposeOpts, ...opened.expose });
    let archive;
    try {
        archive = opened.attach(exposed);
    }
    catch (error) {
        exposed.close();
        throw error;
