...
function schedule(drain, flush) {
    ...
    if (drain == "immediate") {
        (0, defer_immediate_1.deferImmediate)(flush);
        return;
    }
...
function defineOwnValue(target, key, value) {
    const descriptor = Array.isArray((0, reactive_1.toRaw)(target)) && key == 'length' ? { value } : {
        configurable: true,
        enumerable: true,
        writable: true,
        value,
...
function replaceRoot(root, value) {
    value = (0, reactive_value_1.prepareReactiveValue)(value, reactive_1.toRaw);
    for (const k of Reflect.ownKeys(root)) {
        if (Array.isArray((0, reactive_1.toRaw)(root)) && k == 'length')
            continue;
...
function createEachListen(store, opts = {}, rawValues = false) {
    ...
    return (0, Listen_1.createListen)((emit) => {
        ...
        function emitKey(key) {
            ...
            if (exists)
                ...
                known.delete(key);
            const source = rawValues ? raw : store.state;
            emit(key, exists ? source[key] : undefined, { path: [key] });
        }
...
function installStoreEachRawSource(store) {
    Object.defineProperty(store, observe_private_1.STORE_EACH_RAW_SOURCE, {
        value: function createStoreEachRawListen() { return createEachListen(store, {}, true); },
    });
}
...
function dataOnlyPath(root, path) {
    let cur = (0, reactive_1.toRaw)(root);
    for (const k of path) {
        if (!isObj(cur))
            return true;
        ...
        if (!('value' in descriptor))
            return false;
        cur = (0, reactive_1.toRaw)(descriptor.value);
    }
    return true;
}
...
function markPhantom(entry, phantom) {
    if (entry.phantom == phantom)
        return;
    entry.phantom = phantom;
    for (let cur = entry; cur; cur = cur.parent)
        cur.phantoms += phantom ? 1 : -1;
}
function getNodeEntry(store, path) {
    ...
    if (parent) {
        ...
        if (!readRawAt(store._state, path).exists)
            markPhantom(entry, true);
    }
    else
...
function writtenNodeEntry(store, path) {
    let entry = store._nodeRoot;
    for (const key of path) {
        if (!entry)
            return undefined;
        ...
        if (!child && typeof key == 'string') {
            ...
            if (String(index) == key)
                child = entry.children.get(index);
        }
        entry = child;
    }
    return entry;
...
function pruneCachedPath(store, path) {
    ...
    while (parent && parent.path.length) {
        if (pruneAncestorEntry(store, parent))
            break;
...
function subscribePath(store, path, cb, opts = {}, once = false) {
    ...
    function attach() {
        ...
        offUpdate = (0, reactive_1.onUpdate)(target, () => {
            if (target !== lastValue && !(0, reactive_1.isReactive)(lastValue) && !writtenSinceRead())
                return;
            ...
            if (!valueIsObject && !watchedSelf && sameLeaf(lastValue, value, lastExists, exists)) {
                if (!(0, reactive_1.isReactive)(target) && !done)
                    attach();
                return;
            }
            lastExists = exists;
...
function createSelection(store, base, mask, defaults = {}) {
    ...
    return {
        ...
        on(cb, opts = {}) {
            ...
            const offs = [];
            function closeSelection() { drained.close(); for (const off of offs)
                off(); }
            try {
                for (const path of fullPaths)
                    offs.push(subscribePath(store, path, () => drained.push(), o, false));
        ...
        onEach(cb, opts = {}) {
            ...
            const offs = [];
            function closeEachSelection() { for (const off of offs)
                off(); }
            try {
                for (const path of fullPaths)
                    offs.push(subscribePath(store, path, cb, o, false));
...
function createStoreMirror(remote, initial = {}, opts = {}) {
    ...
    async function sync(mask, subOpts = { current: true }) {
        ...
        let closed = false;
        async function pullActiveMask(mask) {
            if (closed)
                return;
            const snap = await remote.get(mask);
            if (!closed)
        ...
        const drained = createDrained(function pullQueuedStoreMask() {
            ...
            chain = chain.then(function pullNextStoreMask() { return pullActiveMask(nextMask); })
                .catch(function reportActivePull(error) { if (!closed)
                report(error); });
        }, subOpts.drain);
