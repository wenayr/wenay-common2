...
exports.RpcBinaryCallbackRefValue = RpcBinaryCallbackRefValue;
exports.createRpcBinaryCallbackRef = createRpcBinaryCallbackRef;
exports.rpcBinaryCallbackRefId = rpcBinaryCallbackRefId;
exports.trustRpcBinaryLeaf = trustRpcBinaryLeaf;
exports.rpcBinaryNativeOwnStateError = rpcBinaryNativeOwnStateError;
exports.rpcBinaryRegExpV1Error = rpcBinaryRegExpV1Error;
...
function RpcBinaryCallbackRefValue() { }
function createRpcBinaryCallbackRef(id) {
    if (!Number.isSafeInteger(id) || id < 0) {
        throw new RangeError('rpc binary callback ref: id must be a non-negative safe integer');
    }
    const value = Object.create(RpcBinaryCallbackRefValue.prototype);
    value[RPC_BINARY_CALLBACK_REF] = id;
    return Object.freeze(value);
}
function rpcBinaryCallbackRefId(value) {
    if (value == null || typeof value != 'object')
        return undefined;
    const prototype = Object.getPrototypeOf(value);
    if (prototype != Object.prototype
        && prototype != RpcBinaryCallbackRefValue.prototype)
        return undefined;
    if (!own.call(value, RPC_BINARY_CALLBACK_REF))
        return undefined;
    const keys = Reflect.ownKeys(value);
    if (keys.length != 1 || keys[0] !== RPC_BINARY_CALLBACK_REF)
        return undefined;
    const id = value[RPC_BINARY_CALLBACK_REF];
...
const sharedArrayBufferGrowableGetter = typeof SharedArrayBufferConstructor == 'function'
    ...
    : undefined;
function trustRpcBinaryLeaf(value) {
...
function rpcBinaryNativeOwnStateError(value, kind, typedArrayItems) {
    if (kind == 'RegExp') {
        const keys = Reflect.ownKeys(value);
...
function rpcBinaryRegExpV1Error(source, flags) {
