...
function commitEncodeTransaction(registry, candidates, announced, transaction) {
...
function writeGeneric(writer, value, depth, encodeGeneric) {
...
function writeSchemaRunPayload(writer, schema, rows, registry, staged, encodeGeneric, counters, depth) {
    for (let fieldIndex = 0; fieldIndex < schema.fields.length; fieldIndex++) {
        ...
        switch (field.kind) {
            ...
            case FIELD_KIND.GENERIC:
                for (const row of rows) {
                    writeGeneric(writer, fieldValue(schema, row, fieldIndex), depth + 1, encodeGeneric);
                    counters.generic++;
                }
                break;
            default:
...
function createRpcBinarySchemaCodec(options) {
    ...
    function buildDataWire(value, transaction, alreadyIncluded = new Set(), compoundStack = new Set(), depth = 0, trustedInput = false) {
        ...
        function encodeNestedGeneric(value, nestedDepth) {
            ...
            try {
                if (value != null && typeof value == 'object' && !Array.isArray(value)) {
                    ...
                    if (dictionary && dictionary.keys.length > MAX_FIELDS) {
                        const valuesWire = buildDataWire(dictionary.values, transaction, included, compoundStack, nestedDepth, trustedInput);
                ...
                const dataWire = buildDataWire(value, transaction, included, compoundStack, nestedDepth, trustedInput);
    ...
    function prepareEncodeMode(value, rootDepth, trustedInput) {
        if (pendingEncode) {
            throw new Error(config.label + ': prepared encode must be committed or rolled back');
        }
        try {
            const transaction = makeEncodeTransaction();
            const wire = buildDataWire(value, transaction, new Set(), new Set(), rootDepth, trustedInput);
    ...
    function prepareEncodeTrusted(value, rootDepth = 0) {
        return prepareEncodeMode(value, rootDepth, true);
    }
    function measureEncodeMode(value, rootDepth, trustedInput) {
        if (pendingEncode) {
            throw new Error(config.label + ': prepared encode must be committed or rolled back');
        }
        try {
            return buildDataWire(value, makeEncodeTransaction(), new Set(), new Set(), rootDepth, trustedInput).byteLength;
        }
        catch (error) {
            return labeledError(config.label, error);
        }
    ...
    function measureEncodeTrusted(value, rootDepth = 0) {
