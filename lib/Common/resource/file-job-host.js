...
function createFileJobHost(deps) {
    ...
    async function confirmUpload(account, fileId) {
        ...
        try {
            return copyFile(await confirmation);
        }
        finally {
            confirmations.delete(fileId);
        }
    }
    ...
    function startJob(account, fileId, input) {
        if (closed)
            throw new Error('file process: host closed');
    ...
    function cancelJob(account, jobId) {
        if (closed)
            throw new Error('file job cancel: host closed');
        const job = store.state.jobs[jobId];
    ...
    async function download(account, fileId) {
        if (closed)
            throw new Error('file download: host closed');
