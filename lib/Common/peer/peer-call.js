...
function createCallManager(deps) {
    ...
    async function onRing(env) {
        if (calls.has(env.pair) || pendingIncoming.has(env.pair))
            return;
        ...
        if (pendingIncoming.get(env.pair) != env)
            return;
    ...
    const offSignals = port.signals.on(function onCallSignal(env) {
        ...
        if (!live) {
            if (env.type == 'hangup' && pendingIncoming.get(env.pair)?.from == env.from)
                pendingIncoming.delete(env.pair);
            return;
        }
        if (env.from != live.handle.peer)
            return;
        if (env.type == 'accept') {
            live.activate();
            return;
