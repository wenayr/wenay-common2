...
import type {MiniScaleHost, MiniTickState} from './mini-scale-host'

type MiniBrowser = ReturnType<MiniScaleHost['browserFragment']>
type MiniReader = ReturnType<MiniScaleHost['readFragment']>
type MiniPrincipal = ReturnType<ReturnType<MiniScaleHost['scaleConnection']>['auth']['resolveAuth']>['object']
type MiniNodePrincipal = Pick<MiniPrincipal, 'whoami' | 'commands'>

export type MiniScaleDemoDeps = {
    element: (id: string) => HTMLElement
    log: (line: string) => void
    /** clients.app.func of the main stand connection. */
    app: {miniScale: ClientAPIAll<MiniBrowser>}
    tab: string
...
export function setupMiniScaleDemo(deps: MiniScaleDemoDeps) {
    ...
    let session: Awaited<ReturnType<MiniScaleDemoDeps['app']['miniScale']['identity']['login']>> | null = null
    /** Hub token provider: connect waves present the session, notices renew it. */
    async function scaleToken({reason}: Parameters<RpcTokenProvider>[0]) {
    ...
    function watchAuth(name: string, authHub: Pick<ReturnType<typeof createRpcClientHub>, 'authListen'>) {
        return authHub.authListen(function onScaleAuthEvent(event) {
    ...
    const scaleHub = createRpcClientHub(
        ...
        r => ({scale: r<MiniPrincipal>('scale')}) as const,
    ...
    function createNodeHub(node: NodeDirectoryView) {
        return createRpcClientHub(
            () => io(node.url, {forceNew: true, transports: ['websocket', 'polling'], auth: {tab: tab + '-mini'}}),
            r => ({app: r<{miniScale: MiniReader}>('app'), scale: r<{miniScale: MiniNodePrincipal}>('scale')}) as const,
            {opt: demoRpcOpt, token: scaleToken},
        )
    }
    const nodeLinks = new Map<string, {hub: ReturnType<typeof createNodeHub>, up: boolean}>()
    /** The gated write surface of nodeId as of NOW; null while its link is down. */
    function nodeScale(nodeId: string) {
        const link = nodeLinks.get(nodeId)
        return link?.up ? link.hub.facade.scale.func.miniScale : null
    }
    async function connectNode(node: NodeDirectoryView) {
        ...
        const nodeHub = createNodeHub(node)
        ...
        return {
            remote: clients.app.func.miniScale.replica,
    ...
    async function addTen(repeat: boolean) {
        ...
        try {
            ...
            const text = String(error != null && typeof error == 'object' && 'message' in error ? error.message : error)
    ...
    async function runAdd(surface: NonNullable<ReturnType<typeof writeSurface>['surface']>, via: string, requestId: string) {
        try {
    ...
    function spawnSimReader() {
        const k = ++nextReader
        const hubs = new Set<Pick<ReturnType<typeof createRpcClientHub>, 'close'>>()
        async function connectReaderNode(node: NodeDirectoryView) {
            ...
            const hub = createRpcClientHub(
                ...
                r => ({app: r<{miniScale: MiniReader}>('app')}) as const,
                {opt: demoRpcOpt},
            )
            hubs.add(hub)
    ...
    function render() {
        ...
        if (session) {
            const left = session.expiresAt == undefined ? null : Math.max(0, Math.round((session.expiresAt - Date.now()) / 1000))
            identityView.textContent = left == null ? `${session.account} · token has no expiry`
                : `${session.account} · token expires in ${left}s (auto-renews)`
        } else {
