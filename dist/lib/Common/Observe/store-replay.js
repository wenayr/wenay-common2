...
function subscribeDecodedReplayLine(line, decode, cb) {
    let upstream;
    ...
    try {
        upstream = line.on(decodeLiveStoreReplay);
    }
    catch (error) {
        failDecode(error);
    }
...
function decodeStoreReplayWireRemote(remote, decode, lifecycleSource = remote) {
