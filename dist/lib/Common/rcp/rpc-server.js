...
function createServer(socket, key, target, hooks, limits, auth, opt, debug = false) {
    ...
    function transformTree(obj, path = []) {
        let current = obj;
        if (hooks?.resolveTransform && !(0, rpc_dynamic_1.isNoStrict)(current)) {
            current = (0, rpc_scope_1.transformRpcScoped)(hooks, current, (0, rpc_scope_1.rpcScopeFor)(hooks, path));
        }
        ...
        for (const k of Object.keys(current)) {
            ...
            out[k] = typeof v == "function" ? (0, rpc_scope_1.transformRpcScoped)(hooks, v, (0, rpc_scope_1.rpcScopeFor)(hooks, [...path, k]))
    ...
    function createFlowHost(channel, settleScope, resourceScope) {
    ...
    function startAuthTimer(at, fn) {
        return (0, rpc_deadline_1.createRpcDeadline)({ at, fire: fn, unref: true });
    }
    ...
    async function handleServerPacket(incoming) {
        ...
        let resourceScope;
        try {
            resourceScope = (0, rpc_scope_1.rpcScopeFor)(hooks, typeof ref == 'number' ? methodPaths[ref] ?? [] : ref);
            resourceScope?.check();
            let fn, ctx;
                ...
                if (idx !== undefined) {
                    ...
                    for (let i = 0; i < ref.length - 1; i++) {
                        ...
                        if (hooks?.resolveTransform && !(0, rpc_dynamic_1.isNoStrict)(curr))
                            curr = (0, rpc_scope_1.transformRpcScoped)(hooks, curr, resourceScope);
            ...
            if (isPipe) {
                ...
                for (let i = 0; i < steps.length; i++) {
                    ...
                    if (step.type === 'get') {
                        ...
                        const stepArgs = (0, rpc_walk_1.unpack)(step.args, (id, args) => { if (!resourceScope || resourceScope.active())
                            sendCb(channel, id, args); }, id => sendCbEnd(channel, id), lim, createFlowHost(channel, settleScope, resourceScope));
                ...
                const args = (0, rpc_walk_1.unpack)(rawArgsOrSteps, (id, values) => { if (!resourceScope || resourceScope.active())
                    sendCb(channel, id, values); }, id => sendCbEnd(channel, id), lim, createFlowHost(channel, settleScope, resourceScope));
                resourceScope?.check();
            ...
            if (wait)
                sendError(channel, reqId, resourceScope && !resourceScope.active() ? resourceScope.error() : e);
        }
        finally {
            if (settleScope)
