...
async function bootRealNode(deps: {
    ...
    const node = createStoreNode<TickState>({
        line: {nodeId: deps.nodeId, storeId: 'rs-readers', originId: 'rs-readers-origin'},
        ...
        serve: {onConnection(handler) { ioServer.on('connection', handler) }, wrap: fragment => ({svc: fragment})},
        onLeave: () => {},
        log: () => {},
    })
...
function spawnSimReader(k: number, directory: ReturnType<typeof createNodeDirectory>) {
    ...
    const reader = createClusterClient<TickState>({
        line: {storeId: 'rs-readers', originId: 'rs-readers-origin', nodeId: 'sim-' + k, initial: {}},
        roster: directory.api!,
        connect: connectReaderNode,
        placement: {label: `sim #${k}`, balance: {checkMs: 60, cooldownMs: 250, moveChance: 1}},
        log: () => {},
    })
    return {
        k,
