...
async function bootRealNode(deps: {
    ...
    const node = createStoreNode<TickState>({
        line: {nodeId: deps.nodeId, storeId: 'mx-readers', originId: 'mx-readers-origin'},
        ...
        serve: {onConnection(handler) { ioServer.on('connection', handler) }, wrap: fragment => ({miniScale: fragment})},
        onLeave: () => {},
        log: () => {},
    })
...
function spawnDemoReader(k: number, directory: ReturnType<typeof createNodeDirectory>, forceNew: boolean) {
    ...
    const reader = createClusterClient<TickState>({
        line: {storeId: 'mx-readers', originId: 'mx-readers-origin', nodeId: 'mx-sim-' + k, initial: {}},
        roster: directory.api!,
        connect: connectReaderNode,
        placement: {label: `mx #${k}`, balance: {checkMs: 60, cooldownMs: 250, moveChance: 1}},
        log: () => {},
    })
    return {
        k,
        reader,
        routeNodeId: () => reader.view.route(),
        close() {
            try { reader.close() }
            finally {
...
async function main() {
    ...
    function setWeight(nodeId: string, weight: number) {
        ...
        if (row) directory.control.patch(nodeId, {weight})
    }
