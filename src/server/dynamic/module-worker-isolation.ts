import {
    assertVerifiedModuleArtifact,
    VerifiedModuleArtifact,
} from '../../Common/dynamic/module-verifier'
import {ModuleIsolationOpenInput, ModuleIsolationPort} from './module-isolation'
import {
    createModuleWorkerSession,
    MODULE_WORKER_VERIFIED_SOURCE,
    ModuleWorkerSessionDeps,
} from './module-worker'

// =====================================================================
// Verified bundled-factory artifact -> worker_threads session
// =====================================================================
...
function decodeVerifiedSource(artifact: VerifiedModuleArtifact) {
...
function runtimeRangeAllowsNode(range: string) {
    ...
    if (match[1] == '>=') return compareVersions(current, required) >= 0
    if (match[1] == '^') return current[0] == required[0] && compareVersions(current, required) >= 0
    if (match[3] == undefined) return current[0] == required[0]
    return compareVersions(current, required) == 0
}

function assertSupportedManifest(artifact: VerifiedModuleArtifact) {
...
export function createModuleWorkerIsolation(deps: ModuleWorkerIsolationDeps = {}) {
    function open(input: ModuleIsolationOpenInput) {
        const {artifact, candidateId, candidateGeneration} = input
        assertVerifiedModuleArtifact(artifact)
        assertSupportedManifest(artifact)
        const manifest = artifact.manifest
        const maximumCallTimeout = Math.max(
            manifest.budget.callTimeoutMs,
            manifest.budget.warmupTimeoutMs,
            manifest.health.timeoutMs,
        )
        const workerDeps: ModuleWorkerSessionDeps = {
            verified: {
                verification: MODULE_WORKER_VERIFIED_SOURCE,
                moduleId: manifest.moduleId,
                version: manifest.version,
                contentHash: manifest.contentHash as `sha256:${string}`,
                source: decodeVerifiedSource(artifact),
            },
            candidateId,
            bindingGeneration: candidateGeneration,
            startupTimeoutMs: Math.min(deps.startupTimeoutMs ?? manifest.budget.warmupTimeoutMs,
                manifest.budget.warmupTimeoutMs),
        ...
        return createModuleWorkerSession(workerDeps)
    }
