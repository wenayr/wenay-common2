...
// =====================================================================
// Shared service runtime. Env is a HOST concern (doc/DYNAMIC-RUNTIME.md): the domain
...
/** Socket.IO uses the URL path as a namespace, so service addresses must be bare origins. */
export function servicePublicUrl(value: string | undefined) {
    if (value == undefined) return undefined
    let url: URL
    try { url = new URL(value) } catch { throw new Error('service public URL must be an absolute HTTP(S) origin') }
