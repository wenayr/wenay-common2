// =====================================================================
// panel — the generic role panel of a service, derived from the definition
// =====================================================================
// Shared service runtime. One static page for EVERY audience of a service: it logs
// in through /api/<name>/login (when the definition declares access.login),
// asks /me what this principal may read and call, polls the readable views
// through the documented GET routes and sends commands through the
// documented POST routes with a fresh requestId. Zero new wire: everything
// on this page is a route the OpenAPI document already describes. A cook,
// a courier, a manager and the owner all open the SAME URL and see
// different panels — the difference is the token, decided server-side.
// Rendering is generic (records → tables); a product replaces this page
// with its own UI over the same routes.

export type ServicePanelSpec = {
    name: string
    basePath: string
    views: {name: string, allow: 'public' | readonly string[]}[]
    commands: {name: string, allow?: readonly string[], example: unknown}[]
    /** The login form fields (the definition's access.login schema keys), or null without credential login. */
    login: {fields: string[]} | null
}

export function servicePanelPage(spec: ServicePanelSpec) {
