...
// =====================================================================
// Shared service runtime: one runtime value in the service definition is the single
...
// tiny: scalars, field optionality, enums, recursive arrays and objects.
...
/** Array positions hold required values; optionality belongs to object fields. */
export type tArrayItemSpec = tFieldKind | (tCompositeSpec & {optional?: never})

export type tFieldSpec = tFieldKind | `${tFieldKind}?` | (tCompositeSpec & {optional?: true})
    ...
    Sp extends `${infer K extends tFieldKind}?` ? tScalar<K>
        : Sp extends tFieldKind ? tScalar<Sp>
            : Sp extends {enum: readonly (infer E extends string)[]} ? E
                : Sp extends {array: infer Item} ? tSpecValue<Item>[]
...
function specJsonSchema(base: tFieldKind | Exclude<tFieldSpec, string>): object {
    ...
    if ('array' in base) return {type: 'array', items: specJsonSchema(base.array)}
