// =====================================================================
// Peer packet mesh — arbitrary payload routing over reusable connections
// =====================================================================
// Discovery supplies connection capabilities. The mesh opens them, measures
// their cost, exchanges loop-safe path advertisements and forwards packets.
// Transports stay outside: RPC relays, WebRTC, workers and in-process pipes all
// implement the same small session contract.
...
export type PeerPacketRouteAdvertisement = {
    targetId: string
    cost: number
    /** Starts at the advertising peer and ends at targetId. */
    path: string[]
}
...
export type PeerPacketEnvelope<T> = {
    ...
    sequence: number
    ttl: number
    /** Nodes which have already handled the packet, ending at the sender. */
    path: string[]
    payload: T
}
...
export type PeerPacketSession<T> = {
    ...
    messages: {on: (cb: (message: PeerPacketWire<T>) => void) => any}
    ping?: () => unknown | Promise<unknown>
    close: () => void
    onFail?: {on: (cb: (reason?: unknown) => void) => any}
}

/** Reusable capability; `connect` may be called again after a failure. */
export type PeerPacketOffer<T> = {
    id: string
    peerId: string
    connect: () => PeerPacketSession<T> | Promise<PeerPacketSession<T>>
    /** Static additive cost. Measured RTT is added to it. */
    priority?: number
}

export type PeerPacketOfferSource<T> = {
    list: () => readonly PeerPacketOffer<T>[]
    changes: {on: (cb: (offers: readonly PeerPacketOffer<T>[]) => void) => any}
}
...
export type PeerPacketSendResult = {
    /** `ok` confirms acceptance by the selected next hop, not destination receipt. */
    ok: boolean
    packetId: string
    targetId: string
    nextHopId?: string
...
export type PeerPacketMeshDeps<T> = {
    ...
    now?: () => number
    /** Authorizes the immediate session peer; origin/path need application-level provenance. */
...
function requiredId(value: unknown, label: string) {
    if (typeof value != 'string' || !value.trim()) throw new Error('peer packet mesh: ' + label + ' is required')
    return value.trim()
}
...
function nonNegativeNumber(value: unknown, label: string) {
    if (!finiteCost(value)) throw new Error('peer packet mesh: ' + label + ' must be a non-negative finite number')
    return value as number
}

function normalizeOffer<T>(offer: PeerPacketOffer<T>) {
    const id = requiredId(offer.id, 'offer id')
    const peerId = requiredId(offer.peerId, 'peer id')
    if (typeof offer.connect != 'function') throw new Error('peer packet mesh: offer connect must be a function')
    if (offer.priority != undefined) nonNegativeNumber(offer.priority, 'offer priority')
    return {...offer, id, peerId}
}

function closeSession(session: {close: () => void} | null | undefined) {
    try { session?.close() } catch {}
}

function sameRoute(a: PeerPacketRoute | undefined, b: PeerPacketRoute | undefined) {
    return !!a && !!b && a.targetId == b.targetId && a.nextHopId == b.nextHopId &&
        a.offerId == b.offerId && a.cost == b.cost && a.path.join('\u0000') == b.path.join('\u0000')
}

// =====================================================================
// Dynamic offer registry
// =====================================================================

export function createPeerPacketOffers<T>(initial: readonly PeerPacketOffer<T>[] = []) {
    const offers = new Map<string, PeerPacketOffer<T>>()
    const [emitChanges, changes] = listen<[readonly PeerPacketOffer<T>[]]>()

    function publish() {
        emitChanges(Array.from(offers.values()))
    }

    function upsert(offer: PeerPacketOffer<T>) {
        const stored = normalizeOffer(offer)
        offers.set(stored.id, stored)
        publish()
        return function removeThisOffer() {
            if (offers.get(stored.id) != stored) return
            offers.delete(stored.id)
            publish()
        }
    }

    function replace(next: readonly PeerPacketOffer<T>[]) {
        offers.clear()
...
export function createPeerPacketMesh<T>(deps: PeerPacketMeshDeps<T>) {
    ...
    if (seenLimit < 1) throw new Error('peer packet mesh: seenLimit must be at least 1')
    ...
    const [emitPacket, packets] = listen<[T, PeerPacketMeta]>()
    const [emitRoutes, routeChanges] = listen<[readonly PeerPacketRoute[]]>()
    const [emitStatus, statusChanges] = listen<[readonly PeerPacketRouteStatus[]]>()
    const entries = new Map<string, OfferEntry>()
    const seen = new Map<string, number>()
    const stats: PeerPacketMeshStats = {
        sent: 0,
        forwarded: 0,
        delivered: 0,
        duplicates: 0,
    ...
    let routes = new Map<string, PeerPacketRoute>()
    let routeVersion = 0
    let sequence = 0
    let closed = false

    function createOfferEntry(offer: PeerPacketOffer<T>) {
        return {
            offer,
            state: 'closed' as tPeerPacketRouteState,
            session: null as PeerPacketSession<T> | null,
            advertisements: new Map<string, PeerPacketRouteAdvertisement>(),
            receivedVersion: -1,
            rtt: null as number | null,
            error: null as string | null,
            generation: 0,
            offMessages: null as any,
            offFail: null as any,
    ...
    type OfferEntry = ReturnType<typeof createOfferEntry>

    function linkCost(entry: OfferEntry) {
        return Math.max(0, entry.offer.priority ?? 0) + (entry.rtt ?? 0)
    }

    function publicStatus(entry: OfferEntry): PeerPacketRouteStatus {
        return {
            id: entry.offer.id,
    ...
    function routeCandidates(targetId: string, visited: readonly string[] = [], hopBudget = maxHops) {
        const candidates: Array<{route: PeerPacketRoute, entry: OfferEntry}> = []
        const visitedSet = new Set(visited)
        for (const entry of entries.values()) {
            if (entry.state != 'open' || !entry.session || visitedSet.has(entry.offer.peerId)) continue
            const direct = entry.offer.peerId == targetId
                ? {targetId, cost: 0, path: [entry.offer.peerId]} as PeerPacketRouteAdvertisement
                : entry.advertisements.get(targetId)
            if (!direct) continue
            if (direct.path.length > hopBudget) continue
            if (direct.path.some(id => visitedSet.has(id))) continue
            const cost = linkCost(entry) + direct.cost
            if (!finiteCost(cost)) continue
            candidates.push({
                entry,
                route: {
                    targetId,
    ...
    function recomputeRoutes() {
        ...
        for (const entry of entries.values()) {
            ...
            for (const target of entry.advertisements.keys()) targets.add(target)
        }
        targets.delete(nodeId)
        const next = new Map<string, PeerPacketRoute>()
        for (const target of targets) {
            const best = routeCandidates(target)[0]
            if (best) next.set(target, best.route)
        }
        let changed = next.size != routes.size
        if (!changed) {
            for (const [target, route] of next) {
                if (!sameRoute(route, routes.get(target))) { changed = true; break }
            }
        }
    ...
    function advertisedRoutes(entry: OfferEntry) {
        const out: PeerPacketRouteAdvertisement[] = [{targetId: nodeId, cost: 0, path: [nodeId]}]
        for (const route of routes.values()) {
            // Path-vector poison reverse: never advertise a path back to a peer
            // which is already in that path.
            if (route.path.includes(entry.offer.peerId)) continue
            out.push({targetId: route.targetId, cost: route.cost, path: [...route.path]})
        }
        return out
    }

    async function sendWire(entry: OfferEntry, message: PeerPacketWire<T>) {
        if (entry.state != 'open' || !entry.session) return false
        const generation = entry.generation
        const session = entry.session
        try {
            const accepted = await session.send(message)
            if (generation != entry.generation || entry.state != 'open' || entry.session != session) return false
            if (accepted == false) throw new Error('transport rejected the packet')
            return true
        } catch (error) {
            if (generation == entry.generation && entry.state == 'open' && entry.session == session) failEntry(entry, error)
            return false
        }
    }

    function sendRoutes(entry: OfferEntry) {
        const message: PeerPacketRouteMessage = {
            protocol: 1,
            kind: 'routes',
            meshId,
            from: nodeId,
    ...
    function broadcastRoutes() {
        for (const entry of entries.values()) if (entry.state == 'open') sendRoutes(entry)
    }

    function seenKey(originId: string, packetId: string) {
        return JSON.stringify([originId, packetId])
    }

    function remember(originId: string, packetId: string) {
        const key = seenKey(originId, packetId)
        seen.delete(key)
        seen.set(key, now())
        while (seen.size > seenLimit) seen.delete(seen.keys().next().value!)
    }

    function validAdvertisement(route: PeerPacketRouteAdvertisement, peerId: string) {
    ...
    function handleRoutes(entry: OfferEntry, message: PeerPacketRouteMessage) {
        if (message.protocol != 1 || message.meshId != meshId || message.from != entry.offer.peerId ||
            ...
            !Array.isArray(message.routes) || message.routes.length > MAX_ROUTE_ADVERTISEMENTS) return
        ...
        for (const route of message.routes) {
            if (!validAdvertisement(route, entry.offer.peerId) || route.targetId == nodeId) continue
            entry.advertisements.set(route.targetId, {targetId: route.targetId, cost: route.cost, path: [...route.path]})
        }
        recomputeRoutes()
        // If our first advertisement raced before the other side subscribed,
        // answer its first table once. Versions stop this becoming a ping-pong.
        if (first) sendRoutes(entry)
    }

    async function forward(packet: PeerPacketEnvelope<T>): Promise<PeerPacketSendResult> {
        if (packet.ttl <= 0) return {ok: false, packetId: packet.packetId, targetId: packet.targetId, reason: 'ttl'}
    ...
    function validPacket(packet: PeerPacketEnvelope<T>, from: string) {
    ...
    async function handlePacket(entry: OfferEntry, packet: PeerPacketEnvelope<T>) {
        ...
        if (closed || generation != entry.generation || entry.state != 'open' || entries.get(entry.offer.id) != entry) return
    ...
    function clearEntrySession(entry: OfferEntry) {
    ...
    function scheduleReconnect(entry: OfferEntry) {
        if (closed || entry.reconnect || !entries.has(entry.offer.id)) return
    ...
    async function probeEntry(entry: OfferEntry) {
        if (entry.state != 'open' || !entry.session?.ping) return
        const generation = entry.generation
        const started = now()
        ...
        try {
            await Promise.race([
                ...
                new Promise<never>(function rejectSlowPing(_resolve, reject) {
                    timeout = timer(pingTimeoutMs, function pingTimedOut() { reject(new Error('ping timeout')) })
                }),
            ])
            if (generation != entry.generation || entry.state != 'open') return
        ...
        Promise.resolve().then(function connectPacketRoute() {
            ...
            try {
                if (!session || typeof session.send != 'function' || typeof session.messages?.on != 'function' ||
                    typeof session.close != 'function') throw new Error('connected session contract is invalid')
                ...
                const offFail = session.onFail?.on(function packetRouteFailed(reason?: unknown) { failEntry(entry, reason) }) ?? null
                if (generation != entry.generation || entry.state != 'open') {
    ...
    function setOffers(next: readonly PeerPacketOffer<T>[]) {
        const replacements = new Map<string, PeerPacketOffer<T>>()
        for (const offer of next) {
            const stored = normalizeOffer(offer)
    ...
    async function send(target: string, payload: T, opts: {packetId?: string, ttl?: number} = {}) {
        ...
        if (closed) return {ok: false, packetId, targetId, reason: 'rejected'} as PeerPacketSendResult
        sequence = seq
        if (seen.has(seenKey(nodeId, packetId))) return {ok: false, packetId, targetId, reason: 'rejected'} as PeerPacketSendResult
        const ttl = opts.ttl == undefined ? maxHops : nonNegativeInteger(opts.ttl, 'ttl')
        const packet: PeerPacketEnvelope<T> = {
            protocol: 1,
            kind: 'packet',
            meshId,
            packetId,
        ...
        if (targetId == nodeId) {
            ...
            return {ok: true, packetId, targetId, path: [nodeId]} as PeerPacketSendResult
        }
        return forward(packet)
    }

    async function broadcast(targets: readonly string[], payload: T, opts: {ttl?: number} = {}) {
