...
// Enumerating every own key of a large TypedArray enumerates every numeric
...
export type BinaryValueCodecOptions = {
    ...
    /** Internal encoded-tree ceiling. RPC adds room for protocol wrappers while
     * application RpcLimits continue to count only the business value. */
    maxDepth?: number
...
const TYPED_ARRAY_SPEC = [
    ...
    // Float16Array is intentionally not part of protocol v1: it is absent from
    // supported Node 16 runtimes, so conditional emission would desynchronize
    // peers which negotiated the same protocol version.
] as const
...
// TextDecoder strips an initial BOM by default. Keys and string values are data,
// so U+FEFF must survive byte-for-byte and remain distinct in shape signatures.
...
type tShapeCacheState = {
    shapes: tBinaryShape[]
    bySignature: Map<string, tBinaryShape>
    fieldRefs: number
    keyTextBytes: number
}

type tShapeTransaction = {
    state: tShapeCacheState
    limits: tShapeCacheLimits
    staged: tBinaryShape[]
    stagedBySignature: Map<string, tBinaryShape>
    fieldRefs: number
    keyTextBytes: number
    definitions: number
    references: number
...
type tBinaryCodecConfig = {
    ...
    label: string
    callbackRefs: boolean
    shapeLimits: tShapeCacheLimits | undefined
    maxDepth: number
    maxBinaryBytes: number
    maxWireBytes: number
}

type tBinaryDecodeLimits = {
    maxDepth: number
...
type tFailure = (message: string) => never

const binaryError: tFailure = function throwBinaryError(message) {
    throw new TypeError(message)
}

const binaryRangeError: tFailure = function throwBinaryRangeError(message) {
...
export function replayBinaryNativeOwnStateError(
    ...
    if (kind == 'TypedArray') {
        ...
        if (trustedBinaryLeaves.has(value)) return undefined
        // Above this boundary, standard JavaScript has no way to enumerate only
        // expando keys without also materializing every integer index. The RPC
        // contract therefore validates common shadows separately and treats the
        // indexed binary payload as the transferable value.
        if (typedArrayItems! > MAX_TYPED_ARRAY_OWN_KEY_SCAN_ITEMS) return undefined
...
function hasRegExpInlineModifiers(source: string) {
    let inClass = false
    for (let index = 0; index < source.length; index++) {
        ...
        if (inClass || char != '(' || source[index + 1] != '?') continue
        let cursor = index + 2
        let hasFlag = false
        ...
        if (hasFlag && source[cursor] == ':') return true
    }
    return false
}

function hasRegExpDuplicateNamedGroups(source: string) {
    ...
    for (let index = 0; index < source.length; index++) {
        ...
        if (name.includes('\\') || names.has(name)) return true
        names.add(name)
        index = end
    }
    return false
...
function boundedDecodeLimit(value: number, maximum: number) {
    if (Number.isNaN(value) || value == Infinity) return maximum
    if (!Number.isFinite(value) || value < 0) return 0
    return Math.min(Math.floor(value), maximum)
}

function binaryDecodeLimits(
    config: tBinaryCodecConfig,
    limits?: RpcLimits,
    wireLimits?: BinaryValueWireLimits,
): tBinaryDecodeLimits {
...
function createTypedArrayEntries() {
    const entries: tTypedArrayEntry[] = []
    ...
    for (const spec of TYPED_ARRAY_SPEC) {
        ...
        if (typeof Constructor != 'function' || !Number.isInteger(Constructor.BYTES_PER_ELEMENT)
            || Constructor.BYTES_PER_ELEMENT <= 0) continue
        entries.push({
            code: spec.code,
            name: spec.name,
            Constructor,
...
function typedArrayEntryByName(name: string) {
...
function isNodeBuffer(value: unknown) {
...
function typedArrayEntryForValue(value: ArrayBufferView) {
    if (isNodeBuffer(value)) return typedArrayEntryByName('Uint8Array')
    for (const entry of typedArrayEntries) {
        if ((value as any).constructor == entry.Constructor) return entry
    }
    return undefined
}

function varUintByteLength(value: bigint) {
    let remaining = value
    let bytes = 1
...
function createByteWriter(maxWireBytes: number) {
    ...
    function ensure(extra: number) {
        ...
        // A media leaf often dominates the frame and is followed by a timestamp
        // or closing protocol fields. Exact-sized growth here would copy the
        // complete payload again for that tiny tail.
    ...
    function writeUtf8(value: string, byteLength: number) {
        ensure(byteLength)
    ...
    function writeVarUintNumber(value: number, maxBytes = 8) {
        ...
        let count = 0
        do {
            if (count >= maxBytes) binaryRangeError('number varuint exceeds limit')
    ...
    function writeVarUint(value: bigint, maxBytes = MAX_BIGINT_VARUINT_BYTES) {
        if (value < 0n) binaryRangeError('varuint cannot be negative')
        let remaining = value
        let count = 0
        do {
            if (count >= maxBytes) binaryRangeError('varuint exceeds limit')
    ...
    function finish() {
        // Large frames are immediately wrapped or transferred. Keep their already
        // exact backing allocation instead of copying it once more; trim tiny frames
        // so a short scalar does not retain the 256-byte starting capacity.
        if (position * 2 < bytes.byteLength) return bytes.slice(0, position)
...
function createByteCounter(maxWireBytes: number) {
    ...
    function writeU8(_value: number) {
        advance(1)
    }

    function writeU16(_value: number) {
        advance(2)
    }

    function writeFloat64(_value: number) {
        advance(8)
    }
    ...
    function writeUtf8(_value: string, byteLength: number) {
        advance(byteLength)
    }
    ...
    function writeVarUintNumber(value: number, maxBytes = 8) {
        ...
        if (byteLength > maxBytes) binaryRangeError('number varuint exceeds limit')
        advance(byteLength)
    }
    ...
    function writeVarUint(value: bigint, maxBytes = MAX_BIGINT_VARUINT_BYTES) {
        ...
        if (byteLength > maxBytes) binaryRangeError('varuint exceeds limit')
        advance(byteLength)
    }
...
function createByteReader(bytes: Uint8Array) {
    ...
    function readVarUint(maxBytes: number, label: string, maxValue?: bigint) {
        ...
        while (count < maxBytes) {
            ...
            count++
            if ((byte & 0x80) == 0) {
                if (count > 1 && payload == 0) binaryError(label + ' has non-canonical varuint')
                if (maxValue != undefined && value > maxValue) binaryError(label + ' overflows')
                return value
            }
            shift += 7n
        }
    ...
    function readVarUintNumber(maxBytes: number, label: string, maxValue: number) {
        let value = 0
        let factor = 1
        let count = 0
        while (count < maxBytes) {
            ...
            if (!Number.isSafeInteger(value)) binaryError(label + ' overflows')
            count++
            if ((byte & 0x80) == 0) {
                if (count > 1 && payload == 0) binaryError(label + ' has non-canonical varuint')
                if (value > maxValue) binaryError(label + ' overflows')
                return value
            }
            factor *= 0x80
        }
    ...
    function readSafeInteger() {
        ...
        for (let count = 0; count < MAX_SAFE_INTEGER_VARUINT_BYTES; count++) {
            ...
            if ((byte & 0x80) == 0) {
                ...
                if (count == MAX_SAFE_INTEGER_VARUINT_BYTES - 1) {
                    // Seven base-128 digits occupy 49 low bits. Split the
                    // eighth digit before zigzag decoding so values near
                    // +/-MAX_SAFE_INTEGER never need a transient BigInt.
                    const half = payload * SAFE_INTEGER_ZIGZAG_HIGH_FACTOR
                        + Math.floor(low / 2)
                    const magnitude = (low % 2 == 0 ? half : half + 1)
                    if (!Number.isSafeInteger(magnitude)) binaryError('integer overflows')
...
function writeLength(writer: tByteWriter, value: number) {
    writer.writeVarUintNumber(value, varUintNumberByteLength(value))
}

function readLength(reader: tByteReader, maximum: number, label: string) {
    return reader.readVarUintNumber(
        varUintNumberByteLength(maximum),
        label,
        maximum,
    )
}
...
function utf8ByteLength(value: string) {
    ...
    for (let index = 0; index < value.length; index++) {
        ...
        if (code >= 0xd800 && code <= 0xdbff) {
            const next = value.charCodeAt(index + 1)
            if (!Number.isInteger(next) || next < 0xdc00 || next > 0xdfff) return -1
            bytes += 4
            index++
            continue
        }
...
function writeString(writer: tByteWriter, value: string) {
    if (value.length > MAX_STRING_CODE_UNITS) binaryRangeError('string exceeds code-unit limit')
    ...
    if (byteLength < 0) {
        ...
        for (let index = 0; index < value.length; index++) writer.writeU16(value.charCodeAt(index))
        return
    }
    if (byteLength > MAX_STRING_BYTES) binaryRangeError('string exceeds byte limit')
...
function readUtf8String(reader: tByteReader, limits: tBinaryDecodeLimits) {
    ...
    if (value.length > limits.maxStringCodeUnits) binaryError('string exceeds code-unit limit')
    return value
}

function readUtf16String(reader: tByteReader, limits: tBinaryDecodeLimits) {
    const codeUnits = readLength(reader, limits.maxStringCodeUnits, 'UTF-16 string length')
    const byteLength = codeUnits * 2
    if (byteLength > MAX_STRING_BYTES) binaryError('string exceeds byte limit')
    const encoded = reader.take(byteLength)
    ...
    if (!hasLoneSurrogate(value)) binaryError('UTF-16 string has no lone surrogate')
    return value
}

function readString(reader: tByteReader, limits: tBinaryDecodeLimits) {
    const tag = reader.readU8()
    if (tag == VALUE_TAG.STRING_UTF8) return readUtf8String(reader, limits)
    if (tag == VALUE_TAG.STRING_UTF16) return readUtf16String(reader, limits)
...
function createShapeTransaction(state: tShapeCacheState, limits: tShapeCacheLimits): tShapeTransaction {
    return {
        state,
        limits,
        staged: [],
...
function encodedStringByteLength(value: string) {
    const byteLength = utf8ByteLength(value)
    return byteLength < 0 ? value.length * 2 : byteLength
}

function shapeKeyTextByteLength(keys: readonly string[], maximum = Infinity) {
    let bytes = 0
    for (const key of keys) {
...
function shapeSignature(prototype: 0 | 1, keys: readonly string[]) {
...
function commitShapeTransaction(transaction: tShapeTransaction) {
...
function validateArray(value: unknown[]) {
    ...
    if (value.length > MAX_ARRAY_ITEMS) binaryRangeError('array exceeds item limit')
    for (const key of Reflect.ownKeys(value)) {
        if (typeof key == 'symbol') binaryError('symbol array keys are not supported')
        const descriptor = Object.getOwnPropertyDescriptor(value, key)
        if (!descriptor) binaryError('array property descriptor disappeared')
        if (own.call(descriptor, 'get') || own.call(descriptor, 'set')) {
            binaryError('accessor properties are not supported')
        }
...
function writeBinaryPayload(
    writer: tByteWriter,
    tag: number,
    value: Uint8Array,
    maxBinaryBytes: number,
) {
    if (value.byteLength > maxBinaryBytes) binaryRangeError('binary value exceeds limit')
    writer.writeU8(tag)
...
const writeValue: tWriteValue = function writeBinaryValue(writer, value, depth, active, context) {
    ...
    if (typeof value == 'number') {
        if (Number.isSafeInteger(value) && !Object.is(value, -0)) {
            ...
            if (Number.isSafeInteger(zigzag)) {
                ...
                writer.writeVarUint(
                    integer < 0n ? (-integer * 2n) - 1n : integer * 2n,
                    MAX_SAFE_INTEGER_VARUINT_BYTES,
                )
            }
        } else {
    ...
    if (typeof value == 'bigint') {
        ...
        if (magnitude > MAX_BIGINT_MAGNITUDE) binaryRangeError('BigInt exceeds bit limit')
    ...
    if (typeof value == 'function') binaryError('function values are not supported')
    if (typeof value == 'symbol') binaryError('symbol values are not supported')
    if (typeof value != 'object') binaryError('unsupported value')
    ...
    if (value instanceof Map) {
        ...
        if (value.size > MAX_ARRAY_ITEMS) binaryRangeError('Map exceeds item limit')
        beginActive(active, value)
        try {
    ...
    if (value instanceof Set) {
        ...
        if (value.size > MAX_ARRAY_ITEMS) binaryRangeError('Set exceeds item limit')
        beginActive(active, value)
        try {
    ...
    if (value instanceof ArrayBuffer) {
        ...
        writeBinaryPayload(
            writer,
            VALUE_TAG.ARRAY_BUFFER,
            new Uint8Array(value),
            context.config.maxBinaryBytes,
        )
        return
    }
    ...
    if (ArrayBuffer.isView(value)) {
        ...
        if (!entry) binaryError('non-standard typed arrays are not supported')
        validateNativeOwnState(
            value,
            'TypedArray',
            source.byteLength / entry.bytesPerElement,
        )
        if (source.byteLength > context.config.maxBinaryBytes) binaryRangeError('binary value exceeds limit')
        // The writer snapshots synchronously. Little-endian payloads can go
        // straight into its target buffer instead of making a throw-away copy.
    ...
    try {
        ...
        if (shapes && keys.length <= shapes.limits.maxFieldRefs) {
            ...
            // Key-byte budgets only change when a new shape is staged. Known
            // shapes no longer re-encode every field name on every object.
...
function copyBinary(value: Uint8Array) {
    const copy = new Uint8Array(value.byteLength)
    copy.set(value)
    return copy
}

function copyTypedArrayEndianAdjusted(value: Uint8Array, bytesPerElement: number) {
    const copy = copyBinary(value)
    if (PLATFORM_LITTLE_ENDIAN || bytesPerElement == 1) return copy
    // Reversing each element is the same transform from platform to wire and back.
...
function readBinaryPayload(reader: tByteReader, limits: tBinaryDecodeLimits) {
    const byteLength = readLength(reader, limits.maxBinaryBytes, 'binary length')
    return reader.take(byteLength)
}

function defineDecodedValue(
    ...
    // __proto__ and runtime-added accessors/non-writable properties must remain
    // ordinary own data. Assignment would invoke or inherit them.
    const property = context.property
    property.value = value
    const defined = Reflect.defineProperty(target, key, property)
    property.value = undefined
    if (!defined) binaryError('cannot define decoded object key')
}

type tReadValue = (
    reader: tByteReader,
    depth: number,
    limits: tBinaryDecodeLimits,
    context: tReadContext,
    allowHole?: boolean,
) => unknown
...
function transactionShapeById(transaction: tShapeTransaction, id: number) {
    if (id < transaction.state.shapes.length) return transaction.state.shapes[id]
    return transaction.staged[id - transaction.state.shapes.length]
}

function decodedShapeObject(
    reader: tByteReader,
    depth: number,
    limits: tBinaryDecodeLimits,
    context: tReadContext,
    ...
    const value: Record<string, unknown> = shape.prototype == 1 ? Object.create(null) : {}
    for (const key of shape.keys) {
        defineDecodedValue(
            value,
            key,
            readValue(reader, depth + 1, limits, context),
            context,
            shape.prototype == 1,
        )
    }
...
const readValue: tReadValue = function readBinaryValue(
    reader,
    depth,
    limits,
    context,
    ...
    if (depth > limits.maxDepth) binaryError('maximum depth exceeded')
    const tag = reader.readU8()
    if (tag == VALUE_TAG.NULL) return null
    if (tag == VALUE_TAG.UNDEFINED) return undefined
    if (tag == VALUE_TAG.FALSE) return false
    if (tag == VALUE_TAG.TRUE) return true
    if (tag == VALUE_TAG.ARRAY_HOLE) {
        if (!allowHole) binaryError('array-hole tag outside an array')
        return ARRAY_HOLE
    }
    if (tag == VALUE_TAG.INTEGER) {
        return reader.readSafeInteger()
    }
    if (tag == VALUE_TAG.FLOAT64) return reader.readFloat64()
    if (tag == VALUE_TAG.STRING_UTF8) return readUtf8String(reader, limits)
    if (tag == VALUE_TAG.STRING_UTF16) return readUtf16String(reader, limits)
    if (tag == VALUE_TAG.BIGINT) {
        const sign = reader.readU8()
        if (sign != 0 && sign != 1) binaryError('invalid BigInt sign')
        const magnitude = reader.readVarUint(
            MAX_BIGINT_VARUINT_BYTES,
            'BigInt',
            MAX_BIGINT_MAGNITUDE,
        )
        if (sign == 1 && magnitude == 0n) binaryError('negative zero BigInt is not canonical')
    ...
    if (tag == VALUE_TAG.CALLBACK_REF) {
        ...
        const id = reader.readVarUintNumber(
            MAX_SAFE_INTEGER_VARUINT_BYTES,
            'callback reference',
    ...
    if (tag == VALUE_TAG.ARRAY) {
        ...
        if (reader.remaining() < length) binaryError('truncated array')
        const value = new Array<unknown>(length)
        for (let index = 0; index < length; index++) {
            const item = readValue(reader, depth + 1, limits, context, true)
            // Strict identity is required: loose comparison coerces null-prototype objects.
            if (item !== ARRAY_HOLE) value[index] = item
        }
        return value
    }
    ...
    if (tag == VALUE_TAG.OBJECT) {
        ...
        if (prototype != 0 && prototype != 1) binaryError('invalid object prototype')
        const count = readLength(reader, limits.maxObjectKeys, 'object key count')
        if (reader.remaining() < count * 3) binaryError('truncated object')
        const value: Record<string, unknown> = prototype == 1 ? Object.create(null) : {}
        const keys = new Set<string>()
        for (let index = 0; index < count; index++) {
            const key = readString(reader, limits)
            if (keys.has(key)) binaryError('duplicate object key')
            keys.add(key)
            defineDecodedValue(
                value,
                key,
    ...
    if (tag == VALUE_TAG.OBJECT_SHAPE_DEF) {
        const shapes = context.shapes
        if (!shapes) binaryError('object shape cache is disabled')
        const id = readLength(reader, shapes.limits.maxEntries - 1, 'object shape id')
        const prototype = reader.readU8()
        if (prototype != 0 && prototype != 1) binaryError('invalid object shape prototype')
        const count = readLength(reader, limits.maxObjectKeys, 'object shape key count')
        if (reader.remaining() < count * 3) binaryError('truncated object shape')
        const keys: string[] = []
        const unique = new Set<string>()
        for (let index = 0; index < count; index++) {
            const key = readString(reader, limits)
            if (unique.has(key)) binaryError('duplicate object shape key')
            unique.add(key)
            keys.push(key)
        }
    ...
    if (tag == VALUE_TAG.OBJECT_SHAPE_REF) {
        ...
        if (!shape) binaryError('unknown object shape reference')
        shapes.references++
        return decodedShapeObject(reader, depth, limits, context, shape)
    }
    ...
    if (tag == VALUE_TAG.MAP) {
        ...
        if (reader.remaining() < count * 2) binaryError('truncated Map')
        const value = new Map<unknown, unknown>()
        for (let index = 0; index < count; index++) {
            const key = readValue(reader, depth + 1, limits, context)
            if (value.has(key)) binaryError('duplicate Map key')
    ...
    if (tag == VALUE_TAG.SET) {
        ...
        if (reader.remaining() < count) binaryError('truncated Set')
        const value = new Set<unknown>()
        for (let index = 0; index < count; index++) {
            const item = readValue(reader, depth + 1, limits, context)
            if (value.has(item)) binaryError('duplicate Set value')
            value.add(item)
        }
        return value
    }
...
// ============================================================
// trusted value decode
// ============================================================

function defineTrustedDecodedValue(
    target: Record<string, unknown>,
    key: string,
    value: unknown,
    nullPrototype: boolean,
...
type tTrustedReadContext = {
    config: tBinaryCodecConfig
    shapes: tShapeTransaction | undefined
}

function readTrustedUtf8String(reader: tByteReader, limits: tBinaryDecodeLimits) {
...
function readTrustedUtf16String(reader: tByteReader, limits: tBinaryDecodeLimits) {
...
function readTrustedString(reader: tByteReader, limits: tBinaryDecodeLimits) {
    const tag = reader.readU8()
    if (tag == VALUE_TAG.STRING_UTF8) return readTrustedUtf8String(reader, limits)
    if (tag == VALUE_TAG.STRING_UTF16) return readTrustedUtf16String(reader, limits)
    return binaryError('expected trusted string')
}

function readTrustedValue(
    reader: tByteReader,
    limits: tBinaryDecodeLimits,
    context: tTrustedReadContext,
    allowHole = false,
): unknown {
    const tag = reader.readU8()
    if (tag == VALUE_TAG.NULL) return null
    ...
    if (tag == VALUE_TAG.FALSE) return false
    if (tag == VALUE_TAG.TRUE) return true
    if (tag == VALUE_TAG.ARRAY_HOLE) return allowHole ? ARRAY_HOLE : binaryError('unexpected array hole')
    if (tag == VALUE_TAG.INTEGER) return reader.readSafeIntegerTrusted()
    if (tag == VALUE_TAG.FLOAT64) return reader.readFloat64()
    if (tag == VALUE_TAG.STRING_UTF8) return readTrustedUtf8String(reader, limits)
    if (tag == VALUE_TAG.STRING_UTF16) return readTrustedUtf16String(reader, limits)
    if (tag == VALUE_TAG.BIGINT) {
    ...
    if (tag == VALUE_TAG.REGEXP) {
        return new RegExp(
            readTrustedString(reader, limits),
            readTrustedString(reader, limits),
        )
    }
    if (tag == VALUE_TAG.ARRAY) {
        const length = reader.readVarUintNumberTrusted()
        if (length > limits.maxArrayItems) return binaryRangeError('trusted array exceeds limit')
        const value = new Array<unknown>(length)
        for (let index = 0; index < length; index++) {
    ...
    if (tag == VALUE_TAG.OBJECT) {
        ...
        if (count > limits.maxObjectKeys) return binaryRangeError('trusted object exceeds limit')
        const value: Record<string, unknown> = nullPrototype ? Object.create(null) : {}
        for (let index = 0; index < count; index++) {
            defineTrustedDecodedValue(
                value,
                readTrustedString(reader, limits),
                readTrustedValue(reader, limits, context),
                nullPrototype,
            )
        }
        return value
    ...
    if (tag == VALUE_TAG.OBJECT_SHAPE_DEF) {
        const shapes = context.shapes
        if (!shapes) return binaryError('trusted object shape cache is disabled')
        const id = reader.readVarUintNumberTrusted()
        const prototype = reader.readU8() == 1 ? 1 : 0
        const count = reader.readVarUintNumberTrusted()
        if (count > limits.maxObjectKeys) return binaryRangeError('trusted shape exceeds limit')
    ...
    if (tag == VALUE_TAG.OBJECT_SHAPE_REF) {
        ...
        if (!shape) return binaryError('unknown trusted object shape')
        shapes.references++
    ...
    if (tag == VALUE_TAG.MAP) {
        const count = reader.readVarUintNumberTrusted()
        if (count > limits.maxArrayItems) return binaryRangeError('trusted Map exceeds limit')
        const value = new Map<unknown, unknown>()
        for (let index = 0; index < count; index++) {
            value.set(
                readTrustedValue(reader, limits, context),
    ...
    if (tag == VALUE_TAG.SET) {
        const count = reader.readVarUintNumberTrusted()
        if (count > limits.maxArrayItems) return binaryRangeError('trusted Set exceeds limit')
        const value = new Set<unknown>()
        for (let index = 0; index < count; index++) {
    ...
    if (tag == VALUE_TAG.ARRAY_BUFFER) {
        const byteLength = reader.readVarUintNumberTrusted()
        if (byteLength > limits.maxBinaryBytes) return binaryRangeError('trusted binary exceeds limit')
    ...
    if (tag == VALUE_TAG.TYPED_ARRAY) {
        ...
        if (!entry) return binaryError('unknown typed-array code')
        const byteLength = reader.readVarUintNumberTrusted()
        if (byteLength > limits.maxBinaryBytes) return binaryRangeError('trusted binary exceeds limit')
        const copy = copyTypedArrayEndianAdjusted(
...
function positiveIntegerOption(value: number | undefined, fallback: number, label: string) {
    const resolved = value == undefined ? fallback : value
...
function resolveShapeCacheOptions(value: BinaryValueCodecOptions['shapeCache']) {
    ...
    const maxEntries = integerOption(
        typeof value == 'number' ? value : options?.maxEntries,
        MAX_SHAPE_ENTRIES,
        MAX_SHAPE_ENTRIES,
        'binary shape-cache maxEntries',
    )
    if (maxEntries == 0) return undefined
    return {
        maxEntries,
        maxFieldRefs: integerOption(
            options?.maxFieldRefs,
            MAX_SHAPE_FIELD_REFS,
            MAX_SHAPE_FIELD_REFS,
            'binary shape-cache maxFieldRefs',
        ),
        maxKeyTextBytes: integerOption(
            options?.maxKeyTextBytes,
            MAX_SHAPE_KEY_TEXT_BYTES,
            MAX_SHAPE_KEY_TEXT_BYTES,
            'binary shape-cache maxKeyTextBytes',
        ),
    }
}

function resolveCodecConfig(options: BinaryValueCodecOptions): tBinaryCodecConfig {
...
export function createBinaryValueCodec(options: BinaryValueCodecOptions) {
    ...
    function applyEncodeStats(transaction: tShapeTransaction | undefined) {
    ...
    function applyDecodeStats(transaction: tShapeTransaction | undefined) {
        if (!transaction) return
    ...
    function prepareEncode(value: unknown) {
        if (pendingEncode) {
    ...
    function measureEncode(value: unknown) {
        if (pendingEncode) {
            throw new Error(config.label + ': prepared encode must be committed or rolled back')
        }
        try {
    ...
    function decodeValue(
        ...
        try {
            ...
            for (const expected of config.magic) {
                if (reader.readU8() != expected) binaryError('magic mismatch')
            }
            const version = reader.readU8()
            if (version != config.version) binaryError('unsupported version')
            const shapes = config.shapeLimits
            ...
            if (!reader.done()) binaryError('trailing bytes')
            if (shapes) commitShapeTransaction(shapes)
