...
import {CBar, CBars, CBarsMutable} from '../src/Bars'
...
test('a pushed bar re-derives the tick size', () => {
    ...
    bars.push(bar(2, 15))
    // the assert above narrowed the getter to 10; push() changes it, which TS cannot see
    assert((bars.tickSize as number) === 5, 'after a close of 15 the grid is 5, got ' + bars.tickSize)
})
